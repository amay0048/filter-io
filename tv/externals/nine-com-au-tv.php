...
$urls = array(
"http://www.jump-in.com.au/watch-now/"
);
...
function updatenews($url) {
	...
	$element = $doc->getElementById('catalogue-most-recent');
	// Convert it to a string and load as XML
	$xml = DOMDocument::loadXML($doc->saveXML($element));
	// Create an XPath finder and query on the classname
	$finder = new DomXPath($xml);
	$classname="entry";
	$results = $finder->query("//*[contains(@class, '$classname')]");

	// look for the results within the html snip
