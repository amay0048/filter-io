  ...
  activity:{
    ...
    helper:{
      ...
      notificationUpdater:function () {
        ...
        var interval = window.setInterval(fnInterval, this.notificationWait);
...
//          for(i in latestUpdates)
      ...
      updateNotifier:function (data) {
        ...
        if (data.notificationCount == 0) {
          $notificationsBtn.removeClass('ui-btn-up-b');
          $notificationsBtn.attr('data-theme', 'a');
        } else {
          $notificationsBtn.removeClass('ui-btn-up-a');
          $notificationsBtn.attr('data-theme', 'b');
        }
        if (data.messagesCount == 0) {
          $messagessBtn.removeClass('ui-btn-up-b');
          $messagessBtn.attr('data-theme', 'a');
        } else {
          $messagessBtn.removeClass('ui-btn-up-a');
          $messagessBtn.attr('data-theme', 'b');
        }
        if (data.requestCount == 0) {
          $requestsBtn.removeClass('ui-btn-up-e');
          $requestsBtn.attr('data-theme', 'a');
        } else {
          $requestsBtn.removeClass('ui-btn-up-a');
          $requestsBtn.attr('data-theme', 'e');
        }

        $notificationsBtn.button().trigger('mouseover').trigger('mouseout');
        $messagessBtn.button().trigger('mouseover').trigger('mouseout');
        $requestsBtn.button().trigger('mouseover').trigger('mouseout');
...
//      });
      ...
      if ($form.length > 0) {
        ...
        $form.find('#all').bind('click', function (event) {
  ...
  hegift:{
    indexSend:function (page, jQPageData, $page) {
      ...
      if ($page.find('#gift_amount').length) {
        ...
        $page.find('input[type=checkbox].user_ids').bind('click', function () {
...
//          link: '', // link to be attached to each result
  ...
  store:{
    ...
    removeFromCart: function(item){
      var $item = $(item);
      $item.closest('li').remove(); $.ajax($item.attr('href'), function(){$.mobile.changePage(location.pathname, {reloadPage: true})})
        return false;
    }
  },
