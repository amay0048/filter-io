...
//        alertLog(menu_item);
      ...
      if (isListview && menu_item.data_attrs && menu_item.data_attrs['role'] == 'list-divider') {
        ...
        if(menu_item.data_attrs){
          ...
          iconDiv.setAttribute('class', 'ui-li-icon ui-li-thumb icon-' + menu_item.data_attrs['data-icon']);
          $a.prepend(iconDiv);
        }
      }
  ...
  adCampaign: function(params, $template){
    ...
    $template.find('.close-btn').bind('click', function(e){
  ...
  quickLinks:function (params, $template) {
    var $link = $(document.createElement('a')).attr({href:"javascript://", 'data-role':"button", id:"apptouch_quick_icon", 'data-icon':"plus", 'data-rel':"dialog", 'data-iconpos':"notext", 'data-theme':"b"});
...
//    $form.find('select').attr('data-native-menu', 'false');
    ...
    $calendars.each(function () {
      ...
      $calendarElement.append($('<div />').attr({'data-role':'fieldcontain'}).append($('<fieldset />').attr({'data-role':'controlgroup', 'data-type':'horizontal'}).append($selects)));
...
//            }
      ...
      if (!item.manage || item.manage.length == 0 || !item.manage[0].label)
        ...
        var $dialog = this.layout.$createBase('dialog', {'data-rel':'dialog', 'id':dialog_id, 'data-url':'#' + dialog_id, 'data-title':item.title}, true);
...
//    } empty html
    ...
    $template.find('a').photoSwipe({ enableMouseWheel:false, enableKeyboard:false, enableComment:params.options.canComment, enableTags:params.options.canViewTags  });
    if (active != undefined) {
  ...
  registryWall:function (params, $template) {
    ...
    var FnVClickWE = function(e){
          ...
          switch(action) {
            ...
            case 'unlike': we.unlike(this);break;
            case 'showCommentModal': we.showCommentModal(this);break;
            case 'showShareModal': we.showShareModal(this);break;
            case 'like': we.like(this); break;
            case 'hashtag': we.hashtagActivate(this); break;
          }
      e.preventDefault();
      return false;
        };
    ...
    $template.undelegate('.wall-event', 'vclick', FnVClickWE);
    $template.delegate('.wall-event', 'vclick', FnVClickWE);
    var $wfm = $('#feed-composer-panel');
    $wfm.undelegate('.wall-event-form', 'vclick', FnVClickWEF);
    $wfm.delegate('.wall-event-form', 'vclick', FnVClickWEF);
    $cl.bind('click', function(e){Wall.events.showComposerPanel(this); e.preventDefault();return false;});
    var aht = $template.find('.active_hashtags');
    if(!sessionStorage.getItem('tipcount') || parseInt(sessionStorage.getItem('tipcount')) < 3)
      aht.bind('vclick', function(){
        var $this = $(this);
...
//      $li.find('div.comment-like').controlgroup().controlgroup('refresh');
    ...
    $form.find('#submit').bind('vclick', function (e) {
      ...
      if (body.length > 50 || body.split(' ').join('').length) {
        ...
        $.post($form.attr('action'), {'format':'json', 'id':subject.id, 'type':subject.type, 'body':body}, function (response) {
          var $item;
          if (response.status) {
  ...
  like:function (params, $template) {
    ...
    like_a.bind('vclick', function (e) {
      ...
      $.post(params.like_url, {'format':'json', 'object_id':params.id, 'object':params.type}, function (response) {
...
//        $template.controlgroup().controlgroup('refresh');
    ...
    unlike_a.bind('vclick', function (e) {
      ...
      $.post(params.unlike_url, {'format':'json', 'object_id':params.id, 'object':params.type}, function (response) {
...
//      var $menu_item_icon = menu_item_tpl.querySelector('.tab_icon_map img').cloneNode(true);
    menu_item_tpl.querySelectorAll('.tab_icon_map')[0].innerHTML = '';
    ...
    for (var key in params) {
      ...
      var $tab_icon_map = $menu_item.querySelectorAll('.tab_icon_map')[0];
      $($tab_icon_map).removeClass('count0');
      //      if (menu_item.icons) {
      //        var len = menu_item.icons.length;
      //        var limit = 0;
      //        if(len >= 6){
      //          limit = 6
      //        } else if(len >= 3){
...
//            var iconEl = $menu_item_icon.cloneNode(true);
//            iconEl.setAttribute('src', menu_item.icons[i]);
//            $tab_icon_map.appendChild(iconEl);
      //        }
//          $tab_icon_map.className += ' ' + ['count', limit].join('');
      //      }
      if (menu_item.active) {
  ...
  playlist:function (params, $template) {
    ...
    for (var order in tracks) {
      ...
      trackAudioElement.addEventListener("ended", function (e) {
        var $this = $(this);
        setTimeout(function () {
          $this.closest('li').next('.track').trigger('vclick');
        }, 1000);
...
//        e.preventDefault();
    ...
    createPopupMenu:function (menu, title, from, to) {
      ...
      var $dialog = this.parent.layout.$createBase('dialog', {'data-rel':'dialog', 'id':dialog_id, 'data-url':'#' + dialog_id, 'data-title':title}, true);
    ...
    initExternalUrls:function($el){
      ...
      for(var i = 0; i < linksCount; i ++){
        var link = links[i];
        if(core.helper.isExternalUrl(links[i].href)){
