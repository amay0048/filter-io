  ...
  public function authLoginAction()
  {
    ...
    if (Engine_Api::_()->user()->getViewer()->getIdentity()) {
      ...
      if (null === $this->_helper->contextSwitch->getCurrentContext()) {
        $this->redirect($this->view->url(array(), 'home'));
      }
...
// Tabs {
  ...
  public function tabVideos($active = false)
  {
    ...
    if( $active ) {
      ...
      $this->add($this->component()->itemSearch($form), 10)
        ->add($this->component()->itemList($paginator, 'browseVideoList', array('attrs' => array('class' => 'tile-view'))), 11)
  ...
  public function tabForumPosts($active = false)
  {
    ...
    if ($active)
      $this->add($this->component()->customComponent('itemList', array('items' => $items)), 7)
        ->add($this->component()->paginator($paginator), 8);
    //    } prepare
    return array(
      'showContent' => false,
  ...
  public function tabAlbums($active = false)
  {
    ...
    if( $active ) {
      ...
      $this->add($this->component()->itemSearch($form), 10)
        ->add($this->component()->itemList($paginator, 'browseAlbumList', array('attrs' => array('class' => 'tile-view'))), 11)
        ->add($this->component()->paginator($paginator), 12);
    }
