  ...
  extends Apptouch_Controller_Action_Bridge
{
  ...
  public function indexBrowseAction()
  {
    ...
    if ($paginator->getTotalItemCount()) {
      $this->add($this->component()->itemList($paginator, "browseItemData"))
  ...
  public function indexManageAction()
  {
    ...
    $this->setFormat('manage')
      ->add($this->component()->itemSearch($this->getSearchForm()));
          if ($paginator->getTotalItemCount()) {
            $this->add($this->component()->itemList($paginator, "manageItemData"))
              ->add($this->component()->paginator($paginator));
          } elseif ($search) {
            if($canCreate)
              $this->add($this->component()->tip(
...
//  PlaylistController {
  ...
  public function playlistViewAction()
  {
    ...
    $controlGroup = $this->dom()->new_('div', array('data-role' => 'controlgroup', 'data-mini' => 'true', 'data-type' => 'horizontal', 'style' => 'text-align: right;float:right;'));
