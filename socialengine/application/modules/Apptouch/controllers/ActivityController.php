  ...
  public function notificationsIndexAction()
  {
    ...
    foreach ($notifications as $notification) {
      ...
      if (!$notification->read) {
        ...
        $itemFormat['attrsLi']['data-theme'] = 'b';
//        $itemFormat['attrsA']['class'] = @$itemFormat['attrsA']['class'] . ' ui-btn-active';
    ...
    if(count($updatesFormat) || $itemCount){
      $this->setFormat('browse')
              ->add($this->component()->html($switcherEl . '<br />'));
    }
...
//          'data-inset' => true,
  ...
  public function notificationsMarkreadAction()
  {
    ...
    try {
      ...
      if($viewerId = $viewer->getIdentity()){
        ...
        $this->view->notificationCount = $notificationCount = (int)$notificationsTable->hasNotifications($viewer);
  ...
  public function notificationsUpdateAction()
  {
    ...
    if ($this->view->viewerId = $viewer->getIdentity()) {
      $this->view->notificationCount = $notificationCount = (int)$notificationTbl->hasNotifications($viewer);
      $this->view->requestCount = Engine_Api::_()->getDbtable('notifications', 'activity')->getRequestsPaginator($viewer)->getTotalItemCount();
      $this->view->messagesCount = $messagesCount = (int)Engine_Api::_()->messages()->getUnreadMessageCount($viewer);
      ...
      //==================================
      $session = new Zend_Session_Namespace(get_class($notificationTbl));
      $lastCount = (int)@$session->lastCount;
      $nowCount = $notificationTbl->hasNotifications($viewer);

      $lastCheckTime = @$session->lastCheckTime;
      $nowCheckTime = date('Y-m-d H:i:s');
      if (!$lastCheckTime)
        $lastCheckTime = $nowCheckTime;

      $isNew = ($lastCount < $nowCount);

      $select = $notificationTbl->select();
      $select
        ->where('`date` >= ?', $lastCheckTime)
        ->where('`user_id` = ?', $viewer->getIdentity())
        ->where('`read` = ?', 0);
//      $this->view->select = $select . '';
//      $this->view->lastCheckTime = $lastCheckTime . '';
//      $this->view->nowCheckTime = $nowCheckTime . '';
      $paginator = Zend_Paginator::factory($select);

      $session->lastCheckTime = $nowCheckTime;

      $latestUpdatesFormat = array();
      if($this->_getParam('updateCheckTime', false)){
        $session->lastCheckTime = $nowCheckTime;
      ...
      foreach ($paginator as $notification) {
        ...
        $count ++;
        $latestUpdateFormat = array(
          'content' => $notification . '',
          'photo' => $notification->getSubject()->getPhotoUrl('thumb.normal'),
          'date' => $notification->date,
          'icon' => $this->getNotificationIcon($notification)
        );
        $latestUpdatesFormat[] = $latestUpdateFormat;
      }

      if(!empty($latestUpdatesFormat))
        $this->view->latestUpdates = $latestUpdatesFormat;
      if ($isNew) {
        $session->lastCount = $nowCount;
