...
//    }
  ...
  public function parseAction($action, $params = array())
  {
    ...
    if ($count_pos) {
      ...
      $likeCount = (substr($content, $search_pos, $count_posend - $search_pos));
      if ($likeCount) {
        $content = str_replace('<div class="desc"></div>', '<div class="desc">' . $this->view->translate('like_%s people like it', $likeCount) . '</div>', $content);
      } else {
  ...
  protected function _getWallActivity(Activity_Model_DbTable_Actions $actionsTable, User_Model_User $user, $params = array())
  {
    ...
    if (isset($params['hashtag']) && $params['hashtag']==1) {
      ...
      $fatch_all = $actionsTable->fetchAll($select);
