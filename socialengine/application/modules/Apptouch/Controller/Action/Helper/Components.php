...
class Apptouch_Controller_Action_Helper_Components extends Apptouch_Controller_Action_Helper_Abstract
{
  ...
  {
    $html = $this->_bridge->dom()->new_('ul', array('data-role' => "listview", 'data-theme' => "d", 'data-divider-theme' => "d", 'class' => "component-date"), null);
    $li = $this->_bridge->dom()->new_('li', array('data-role' => "list-divider"), $date['title']);
    if (isset($date['count']))
      $li->append($this->_bridge->dom()->new_('span', array('class' => "ui-li-count"), $date['count']));
    $html->append($li);
  ...
  public function timelineCover()
  {
    ...
    if (!($position = $table->getSetting($user, 'timeline-cover-position'))) {
      $component['position'] = array('top' => 0, 'left' => 0);
    } else {
      $position = unserialize($position);
      $position['top'] = abs($position['top']) - 315;
