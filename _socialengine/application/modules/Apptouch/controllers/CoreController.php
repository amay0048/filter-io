  ...
  extends Apptouch_Controller_Action_Bridge
{
  ...
  public function boardIndexAction()
  {
    ...
    $dashboardHeaderMenus = array('updates' => '0', 'requests' => '0', 'messages' => '0');
...
//          $order_table = Engine_Api::_()->getItemTable('store_order');
          ...
          if ($cart->hasItem()) {
            $dashboardHeaderMenus['store'] = 0;
              ...
              $cartBtn = $this->dom()->new_('a', array(
                  ...
                  'class' => 'store-btn'
              ), count($items));// . ', Total Price - ' . $this->view->locale()->toCurrency($total, $this->_order->currency));
          }
      }
    ...
    if($viewer->getIdentity()){
      ...
      foreach($dashboardHeaderMenus as $key => $menu) {
        $dashboardHeaderMenus[$key] = '1';
      }
    ...
    $this
//      ->setPageTitle($this->view->translate('Dashboard'))
//      ->add($this->component()->html($this->dom()->new_('a', array(
//        'class'           => 'ui-btn-right',
//        'data-role'       => 'button',
//        'data-icon'       => $viewer->getIdentity() ? 'power' : 'login',
//        'data-iconpos'       => 'notext',
//        'data-transition' => 'none',
//        'href'            => $viewer->getIdentity() ? $this->view->url(array(), 'user_logout') : $this->view->url(array(), 'user_login')
//      ), $this->view->translate($viewer->getIdentity() ? 'Sign Out' : 'Sign In'))), null, 'header')
//      ->add($this->component()->html($this->dom()->new_('a', array(
//        'class'           => 'ui-btn-right',
//        'data-role'       => 'button',
//        'data-icon'       => 'arrow-u',
//        'data-ajax'       => 'false',
//        'data-transition' => 'none',
//        'href'            => $this->view->url(array('apptouch-site-mode' => 'standard'), 'default', true)
//      ), $this->view->translate('Full Site'))), null, 'footer')
      ->add($this->component()->itemSearch($searchForm), null, 'content')
      ->add($this->component()->dashboard(), null, 'content')
//      ->add($this->component()->footerMenu(Engine_Api::_()->getApi('menus', 'apptouch')->getNavigation('core_footer')), null, 'footer')
      ->renderContent();
...
//      new Core_Form_Search();
    ...
    if( $query ) {
      ...
      if( $paginator->getTotalItemCount()){
        ...
        $this->add($this->component()->itemList($paginator, 'searchResultItemCustomizer'))
